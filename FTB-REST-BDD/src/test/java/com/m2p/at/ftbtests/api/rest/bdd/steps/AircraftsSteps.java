...
public class AircraftsSteps {
    ...
    @When("client tries to create an Aircraft with null number of seats having manufacturer={string} and model={string}")
    public void createwithNullNumberOfSeats(String manufacturer, String model) throws JsonProcessingException {
        var data = CreateAircraftDto.of()
                .manufacturer(manufacturer)
                .model(model)
                .build();
    ...
    @Then("aircraft data to be with null number of seats and manufacturer={string} and model={string}")
    @Then("returned aircraft data to with null number of seats and manufacturer={string} and model={string}")
    ...
    @Then("aircraft data with null number of seats to be manufacturer={string} and model={string}")
    @Then("returned aircraft data with null number of seats to be manufacturer={string} and model={string}")
    public void verifySingleAircraftDatawithNullSeats(String manufacturer, String model) {
        var lastResponse = storage.getLastApiCallSingleItemResponse();
        assertThat(lastResponse.getManufacturer())
                .as("Seems Aircraft response contained unexpected manufacturer value.")
                .isEqualTo(manufacturer);
